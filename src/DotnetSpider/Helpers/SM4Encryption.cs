...
public static class SM4Encryption
{
    ...
    public static string Encrypt(string msg, string key, string iv, string mode = "ECB", bool isHex = false)
    {
        ...
        var plainTextData = Encoding.UTF8.GetBytes(msg);
        ...
        var keyBytes = Encoding.UTF8.GetBytes(key); // 假设的密钥，实际中应该使用安全的方式生成和存储  
        ...
        var cipherTextData = new byte[cipherMode.GetOutputSize(plainTextData.Length)];
    ...
    public static string Decrypt(string encryptMsg, string key, string iv, string mode = "ECB", bool isHex = false)
    {
        ...
        var decryptedData = new byte[cipherMode.GetOutputSize(cipherTextData.Length)];
        ...
        // 打印解密后的明文  
        var decryptedMsg = Encoding.UTF8.GetString(decryptedData);
        return decryptedMsg;
    }
