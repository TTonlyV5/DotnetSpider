...
public class SM2Encryption
{
    ...
    public static void GenerateSM2KeyPair(out string privateKey, out string publicKey)
    {
        ...
        {
            // 创建SM2密钥对生成器的参数  
            var keyGenParams = new ECKeyGenerationParameters(new ECDomainParameters(CurveParams), secureRandom);
    ...
    public static string Encrypt(string message, string publicKey)
    {
        try
        {
            ...
            //创建公钥参数
            var domainParams = new ECDomainParameters(CurveParams);
            var pubKeyParams = new ECPublicKeyParameters("EC", q, domainParams);
