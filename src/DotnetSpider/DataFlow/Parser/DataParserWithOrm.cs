...
public class DataParserWithOrm<TEntity, TKey> : DataParser
        where TEntity : EntityBase<TKey>, new()
        where TKey : struct
{
    protected ModelWithOrm<TEntity, TKey> Model { get; private set; }

    public override Task InitializeAsync()
    {
        Model = new ModelWithOrm<TEntity, TKey>();
    ...
    protected override Task ParseAsync(DataFlowContext context)
    {
        ...
        if (results.Count > 0)
        {
            AddParsedResultWithOrm(context, results);
        }

        return Task.CompletedTask;
    }

    protected virtual void AddParsedResultWithOrm(DataFlowContext context, IEnumerable<TEntity> results)
    {
        ...
        if (items == null)
        {
            context.AddData(type, results.ToList());
        }
        else
        {
