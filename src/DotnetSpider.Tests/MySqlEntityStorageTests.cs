...
public class MySqlEntityStorageTests(ITestOutputHelper testOutputHelper) : TestBase
{
    ...
    public async Task CreateTableWhenNoSchema()
    {
            ...
            await storage.HandleAsync(context, _ => Task.CompletedTask);
    ...
    public async Task UpdateAllColumns()
    {
                ...
                $"drop table if exists {Escape}test{Escape}.{Escape}createtableprimay{Escape};");
            {
                ...
                dfc2.AddData(typeName,
                    ...
                    {
                        new() { Str1 = "TTT", DateTime = now, DateTimeOffset = now, Double = 888 }
                    });
                var storage2 = CreateStorage(StorageMode.Update);
                await storage2.HandleAsync(dfc2, _ => Task.CompletedTask);
    ...
    public async Task UseTransaction()
    {
            ...
            {
                var storage =
                    (RDEntityStorageBase)CreateStorage(StorageMode.InsertIgnoreDuplicate);
    ...
    public async Task IgnoreCase()
    {
            ...
            {
                var storage = (RDEntityStorageBase)CreateStorage(StorageMode.Insert);
